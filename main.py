...
def main(args):
    ...
    # Define Depth map completion model
    ...
    while True:
        ...
        # Use inferencer Complete depth map
        refin_depth_img, depth_img = inferencer.inference(color_img, depth_img, depth_coefficient = 3, inpainting = True)
        ...
        color_heightmap, depth_heightmap, seg_mask_heightmaps = utils.get_heightmap(
            color_img, refin_depth_img, segment_results['masks'], robot.cam_intrinsics, robot.cam_pose, workspace_limits, heightmap_resolution)
        ...
        logger.save_images(trainer.iteration, color_img, depth_img,refin_depth_img)
        ...
        if not nonlocal_variables['seeking_target']:
            ...
            prev_depth_img = refin_depth_img.copy()
