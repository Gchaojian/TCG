...
class FeatureTunk(nn.Module):
    ...
    def forward(self, color, depth, mask):
        color_feat = self.color_extractor(color)
        depth_feat = self.depth_extractor(depth)
        mask_feat = self.mask_extractor(mask)


        combined_feat = torch.cat((color_feat, depth_feat, mask_feat), dim=1)

        # Input into densenet for calculation
        output = self.dense121(combined_feat)
