...
class reinforcement_net(nn.Module):
    ...
    def __init__(self, use_cuda):
        ...
        # Define the attention modules for push and grasp branches
        in_channels=1024
        ...
        if is_volatile:
            with torch.no_grad():
                ...
                for rotate_idx in range(self.num_rotations):
                    ...
                    unprocessed_features = self.feature_tunk(rotate_color, rotate_depth, rotate_mask)
                    # unprocessed_features.append([unprocessed_features])

                    # Apply CBAM attention
                    interm_feat = self.attention(unprocessed_features)
                    # interm_feat.append([interm_feat])
            ...
            # Compute intermediate features
            self.unprocessed_features = self.feature_tunk(rotate_color, rotate_depth, rotate_mask)
            # self.unprocessed_features.append([self.unprocessed_features])

            # Apply CBAM attention
            self.interm_feat = self.attention(self.unprocessed_features)
            # self.interm_feat.append([self.interm_feat])
